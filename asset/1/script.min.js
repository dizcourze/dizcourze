...
function setLang() {
  fetch("https://www.cloudflare.com/cdn-cgi/trace").then(x => x.text()).then(y => setLangcookie(y));
}
function setLangcookie(y) {
  if (y.indexOf("loc=TW") != -1) {
    setCookie("lang", "TW", 365);
	langTw();
  } else if (y.indexOf("loc=JP") != -1) {
    setCookie("lang", "JP", 365);
	langJp();
  } else if (y.indexOf("loc=KR") != -1) {
    setCookie("lang", "KR", 365);
	langKr();
  } else {
    setCookie("lang", "EN", 365);
